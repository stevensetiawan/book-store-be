...
const registerUser = async (payload: Payload): Promise<User2 | null> => {
  try {
    const result = await pool.query<User2>('INSERT INTO customers (name, email, points, password) VALUES ($1, $2, $3, $4) returning *', [payload.name, payload.email, 1000, payload.hashedPassword]);
...
const findById2 = async (id: number): Promise<User2 | null> => {
  try {
    const result = await pool.query<User2>('SELECT * FROM customers WHERE id = $1 LIMIT 1', [id]);
    return result[0] || null; // Return the first user object or null if result is empty
  } catch (error) {
    console.error('Error finding user by ID:', error);
    throw error;
...
export { registerUser, findOne, findById, findById2 };